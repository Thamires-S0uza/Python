# texto txtA possui respostas
#serve para você testar o seu código
#Na entrega,seu programa deve mostrar apenas as respostas do texto txtB
#Nome: Douglas Turma ADS A
#Nome: Thamires
...
len(x) == 7 and x[-1] not in zumbi and x[0] not in zumbi
...
##txt = ('Abacate, Mamão, Abacaxi')
##ordem = 'znktlpx'
##ordem.index('z')
##ord('A')
##chr(65)
##chr(ordem.index('z') + 65)
###vota é menos 65
textA
def converte(s):
    resp = ''
    ...
    return resp
newA = [converte(p) for p in textA]
newA.sort()
ord('H') - 65
ordem[ord('H') - 65]
def volta(s):
    ...
    for c in s:
        resp += ordem[ord(c) - 65]
    return resp
listaA = [volta(p) for p in newA]
print(listaA)

print('Questão E')

##o*1 + 1*20 + 2*400 + 0*8000 +1*160000 + 2*3200000
##n = 'zmbzmb'
##p20 = 1
##conversão = 0
##for d in n:
##    conversão = conversão + ordem.index(d) * p20
##    p20 = p20 * 20
##    conversão % 42 = 0
##print(convesão)
def numero(s):
